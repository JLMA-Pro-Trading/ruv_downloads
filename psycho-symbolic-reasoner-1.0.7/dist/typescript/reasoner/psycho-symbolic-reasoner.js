...
export class PsychoSymbolicReasoner {
    knowledgeGraph;
    entityIndex; // entity -> triple IDs
    predicateIndex; // predicate -> triple IDs
    reasoningCache;
    startTime;
    constructor() {
    ...
    initializeBaseKnowledge() {
        ...
        for (const triple of baseTriples) {
            this.addKnowledge(triple.subject, triple.predicate, triple.object, { source: 'base-knowledge', confidence: 0.95 });
        }
    }
    /**
     ...
     */
    applyInferenceRules(knowledge, context) {
        const inferences = [];
        ...
        const perfTriples = knowledge.filter(t => t.predicate === 'response-time' ||
     ...
     */
    synthesizeResult(query, knowledge, inferences) {
        const queryLower = query.toLowerCase();
        // Performance-related queries
        if (queryLower.includes('fast') || queryLower.includes('performance')) {
            const perfData = knowledge.filter(t => t.predicate === 'response-time' ||
        ...
        if (queryLower.includes('how') || queryLower.includes('work')) {
            const archData = knowledge.filter(t => t.predicate === 'combines' ||
                t.predicate === 'uses' ||
     ...
     */
    analyzeReasoningPath(query, showSteps = true, includeConfidence = true) {
        ...
        return reasoningPromise.then(reasoning => {
            ...
            const bottleneck = reasoning.steps.reduce((max, step) => step.duration_ms > max.duration_ms ? step : max);
