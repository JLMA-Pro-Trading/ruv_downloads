...
export class ClaudeFlowHooks {
   ...
   */
  async preTask(taskDescription: string, taskId?: string): Promise<void> {
   ...
   */
  async postEdit(filePath: string, context: OptimizationContext): Promise<void> {
  ...
  async postTask(
    taskId: string,
    success: boolean,
    metrics?: OptimizationContext['metrics']
  ): Promise<void> {
   ...
   */
  private async loadWarmStartPatterns(taskDescription: string): Promise<void> {
    const { maxPatterns = 5, similarityThreshold = 0.75 } =
      this.config.reasoningbank?.warmStart || {};
   ...
   */
  private async storeInReasoningBank(key: string, data: any): Promise<void> {
    try {
   ...
   */
  private async updateReasoningBankConfidence(
    taskId: string,
    success: boolean
  ): Promise<void> {
    const { successBonus = 0.20, failurePenalty = 0.15 } =
      this.config.reasoningbank?.confidence || {};
   ...
   */
  private async trainNeuralPatterns(taskId: string, success: boolean): Promise<void> {
    try {
   ...
   * Calculate metrics
   */
  private calculateSuccessRate(state: SessionState): number {
  ...
  private calculateAverageConfidence(state: SessionState): number {
...
export const preTask = (desc: string, id?: string) => hooks.preTask(desc, id);
export const postEdit = (file: string, ctx: OptimizationContext) => hooks.postEdit(file, ctx);
export const postTask = (id: string, success: boolean, metrics?: any) =>
  hooks.postTask(id, success, metrics);
