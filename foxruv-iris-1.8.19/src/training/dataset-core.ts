 ...
 */
export class DatasetBuilder<T extends TemporalExample = TemporalExample> {
  /**
  ...
  private splitByTimestamp(
    ...
    if (config.minTrainExamples && training.length < config.minTrainExamples) {
      console.warn(
        `⚠️  Training set has ${training.length} examples, ` +
    ...
    if (config.minValidationExamples && validation.length < config.minValidationExamples) {
      console.warn(
        `⚠️  Validation set has ${validation.length} examples, ` +
  ...
  private splitByRollingWindow(
    ...
    const validation = examples.filter(
      ex => ex.temporal_key > trainThroughKey && ex.temporal_key <= validationEndKey
    )
  ...
  detectTemporalLeakage(training: T[], validation: T[]): boolean {
    ...
    const latestTrain = training.reduce((latest, ex) =>
      ex.temporal_key > latest.temporal_key ? ex : latest,
      training[0]
    )

    // Find earliest validation example
    const earliestVal = validation.reduce((earliest, ex) =>
      ex.temporal_key < earliest.temporal_key ? ex : earliest,
      validation[0]
    )
 ...
 */
export function balanceByOutcome<T extends TemporalExample>(
  examples: T[],
  outcomeKey: string = 'is_correct'
): T[] {
 ...
 */
export function balanceByCategory<T extends TemporalExample>(
  examples: T[],
  categoryKey: string
): T[] {
  // Group by category
 ...
 */
export function exportToJSONL<T>(
  examples: T[],
  outputPath: string
): void {
 ...
 */
export function exportSplitToJSONL<T>(
