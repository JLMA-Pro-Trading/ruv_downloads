...
import {
  saveReflexion,
  findSimilarReflexions as findSimilarReflexionsSupabase,
  markReflexionReused,
  getReflexionStats as getReflexionStatsSupabase,
...
export interface TrackedReflexion {
  ...
  actionItems: string[]
  timestamp: Date
  validityScore: number // 0-1, based on recent outcomes
  usageCount: number
...
export interface DriftDetection {
  ...
  validityChange: number // Change in validity score
}
...
export interface ReflexionMonitorConfig {
  dbPath?: string
  validityThreshold?: number // Below this, marked as stale
  driftWindow?: number // Days to look back for drift detection
  minUsageForValidity?: number // Minimum uses before calculating validity
...
export class ReflexionMonitor {
  private db: any
  private config: Required<ReflexionMonitorConfig>
   ...
   */
  private async cacheReflexionLocally(
    id: string,
    project: string,
    expertRole: string,
    experience: string,
   ...
   */
  private async recalculateValidity(reflexionId: string): Promise<void> {
    const db = await this.getDb()
    if (!db) return
    ...
    const stats = stmt.get(reflexionId, cutoff) as { total: number; successful: number }
   ...
   */
  async markAsStale(reflexionId: string, reason: string): Promise<void> {
    const db = await this.getDb()
    if (!db) return
   ...
   */
  async markAsReused(reflexionId: string): Promise<void> {
    // Try Supabase first
   ...
   */
  private async incrementLocalReuseCount(reflexionId: string): Promise<void> {
    const db = await this.getDb()
    if (!db) return
   ...
   */
  async detectDrift(reflexionId: string): Promise<DriftDetection> {
    ...
    const recentValidity = await this.calculateValidityForPeriod(
      reflexionId,
      now - halfWindow,
      now
    )

    const olderValidity = await this.calculateValidityForPeriod(
      reflexionId,
      now - windowMs,
      now - halfWindow
    )
    ...
    if (Math.abs(validityChange) < 0.1) severityLevel = 'low'
    else if (Math.abs(validityChange) < 0.2) severityLevel = 'medium'
    else if (Math.abs(validityChange) < 0.3) severityLevel = 'high'
   ...
   */
  private async calculateValidityForPeriod(
    reflexionId: string,
    startMs: number,
    endMs: number
  ): Promise<number> {
    ...
    const stats = stmt.get(reflexionId, startMs, endMs) as {
      total: number
      successful: number
    }

    if (stats.total === 0) return 1.0 // No data, assume valid
   ...
   */
  async findSimilarReflexions(
    reflexionId: string,
    threshold: number = 0.7
  ): Promise<ReflexionComparison> {
    const reflexion = await this.getReflexion(reflexionId)
    ...
    const [supabaseResults, localResults] = await Promise.allSettled([
      ...
      isSupabaseInitialized()
        ? (async () => {
            ...
            const supabaseReflexions = await findSimilarReflexionsSupabase(
              embedding,
              {
   ...
   */
  private async findSimilarReflexionsLocal(
    reflexion: TrackedReflexion,
    threshold: number
  ): Promise<ReflexionComparison> {
    const db = await this.getDb()
    ...
    const candidates = stmt.all(reflexion.project, threshold) as any[]
    ...
    const similarReflexions = candidates
      .map(candidate => {
        ...
        const sharedInsights = reflexionInsights.filter((insight: string) =>
          candidateInsights.includes(insight)
        ).length

        const similarity =
          (sharedInsights * 2) / (reflexionInsights.length + candidateInsights.length)
   ...
   */
  private async generateEmbedding(_reflexion: TrackedReflexion): Promise<number[]> {
   ...
   */
  private extractProjectFromContext(reflexion: ReflexionEntry): string {
    return reflexion.project || (reflexion.context as any)?.project || 'unknown'
  }

  /**
   * Extract ID from Supabase reflexion context
   */
  private extractIdFromContext(reflexion: ReflexionEntry): string {
    return (reflexion.context as any)?.id || reflexion.id
  }
   ...
   */
  async createAdvisory(advisory: ReflexionAdvisory): Promise<void> {
    const db = await this.getDb()
    if (!db) return
    ...
    stmt.run(
      ...
      advisory.severity,
      advisory.message,
      advisory.reflexionId,
      JSON.stringify(advisory.recommendations),
      advisory.timestamp.getTime()
    )
  }
   ...
   */
  async getUnacknowledgedAdvisories(): Promise<ReflexionAdvisory[]> {
    const db = await this.getDb()
    if (!db) return []
   ...
   */
  async acknowledgeAdvisory(advisoryId: number): Promise<void> {
    const db = await this.getDb()
    if (!db) return
   ...
   * Get reflexion by ID
   */
  async getReflexion(id: string): Promise<TrackedReflexion | null> {
    const db = await this.getDb()
    if (!db) return null
   ...
   */
  async getProjectReflexions(project: string, includeStale: boolean = false): Promise<TrackedReflexion[]> {
  ...
  async getStats(project?: string): Promise<{
    ...
    const driftCount = (driftStmt.get(cutoff) as any).count
 ...
 */
export function createReflexionMonitor(config?: ReflexionMonitorConfig): ReflexionMonitor {
