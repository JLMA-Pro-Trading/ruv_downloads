...
export async function scanProject(
  ...
  const allExperts: DiscoveredExpert[] = [];
  ...
  try {
    ...
    throw new Error(
      `Failed to scan project at ${absolutePath}: ${
...
export async function scanFile(
  ...
  const hasTelemetry = imports.some((imp) =>
    imp.importedNames.some((name) => TELEMETRY_IMPORTS.includes(name))
  );
 ...
 */
export function isExpertClass(className: string): boolean {
 ...
 */
export function isPredictionMethod(methodName: string): boolean {
...
export function generateSummary(result: ScanResult): string {
  ...
  result.experts.forEach((expert) => {
    ...
    lines.push(
      `  ${expert.className} (${expert.relativePath}:${expert.lineNumber})`,
      `    Methods: ${expert.methods.length} (${predictionMethods.length} prediction)`,
      `    Telemetry: ${expert.hasTelemetry ? '✓' : '✗'}`,
      `    Supabase: ${expert.hasSupabaseInit ? '✓' : '✗'}`,
      ''
    );
  });
...
export function filterExperts(
  ...
  return experts.filter((expert) => {
    ...
    if (
      criteria.hasSupabaseInit !== undefined &&
    ...
    if (
      criteria.hasPredictionMethods !== undefined &&
