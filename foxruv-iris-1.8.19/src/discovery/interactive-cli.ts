...
export interface InteractiveOptions {
  autoApprove?: boolean;  // Skip prompts in CI/CD
  dryRun?: boolean;
  verbose?: boolean;
}
...
export interface InstrumentationDecision {
  approved: boolean;
  selectedExperts: string[];  // IDs to instrument
...
export class InteractiveCLI {
   ...
   */
  async presentDiscoveries(discoveries: DiscoveredExpert[]): Promise<void> {
    ...
    const byFile = discoveries.reduce((acc, expert) => {
      ...
      return acc;
    }, {} as Record<string, DiscoveredExpert[]>);
  ...
  ): Promise<InstrumentationDecision> {
   ...
   */
  async showCodeChanges(changes: CodeChange[]): Promise<boolean> {
    if (this.options.autoApprove) {
      return true;
    }
   ...
   */
  async confirmInstrumentation(): Promise<boolean> {
    if (this.options.autoApprove) {
      return true;
    }
 ...
 */
export function createProgressIndicator(total: number, message: string = 'Processing') {
