 ...
 * ```
 */
export async function storeExpertSignature(
  expertId: string,
  version: string,
  prompt: string,
  ...
  if (error) throw new Error(`Failed to store expert signature: ${error.message}`);
  return data as ExpertSignature;
}

/**
 * Load the active expert signature for a given expert
 */
export async function loadActiveExpertSignature(
  expertId: string
): Promise<ExpertSignature | null> {
  const supabase = getSupabase();
  const project = getProjectId();
  ...
  return data as ExpertSignature | null;
}

/**
 * Load a specific version of an expert signature
 */
export async function loadExpertSignatureVersion(
  expertId: string,
  version: string
): Promise<ExpertSignature | null> {
  const supabase = getSupabase();
...
export async function getSignatureHistory(
  ...
  if (error) throw new Error(`Failed to load signature history: ${error.message}`);
  return (data as ExpertSignature[] | null) || [];
}
 ...
 */
export async function recordSignatureUpgrade(
  expertId: string,
  fromVersion: string,
  toVersion: string,
  changelog: string,
  ...
  if (error) throw new Error(`Failed to record signature upgrade: ${error.message}`);
  return data;
}
 ...
 */
export async function getSignatureVersionHistory(
  expertId: string
): Promise<SignatureVersion[]> {
  const supabase = getSupabase();
  const project = getProjectId();
