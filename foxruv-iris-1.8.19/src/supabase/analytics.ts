...
import { ModelRunLog, StoredIrisReport } from './types.js';
...
export async function getHealthTrends(
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to get health trends: ${error.message}`);
    if (!data || data.length === 0) return [];

    return (data as StoredIrisReport[]).map(report => ({
...
export async function getSuccessRateTrends(
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to get success rate trends: ${error.message}`);
    if (!data || data.length === 0) return [];

    // Group by hour
    const hourlyMap = new Map<string, { total: number; success: number }>();
    (data as Partial<ModelRunLog>[]).forEach(log => {
...
export async function getLatencyTrends(
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to get latency trends: ${error.message}`);
    if (!data || data.length === 0) return [];
...
export async function getReflexionImpactStats(
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to get reflexion impact stats: ${error.message}`);
    if (!data || data.length === 0) return [];
 ...
 */
export async function getTokenConsumptionTrends(
  projectId: string,
  hours: number = 24
): Promise<Array<{
  time: string;
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to get token consumption trends: ${error.message}`);
    if (!data || data.length === 0) return [];

    // Group by hour
    const hourlyMap = new Map<string, { tokensIn: number; tokensOut: number; cost: number }>();
    (data as Partial<ModelRunLog>[]).forEach(log => {
      const timestamp = new Date(log.timestamp!);
      const hourKey = new Date(
        timestamp.getFullYear(),
      ...
      stats.cost += parseFloat((log.cost_usd as any) || '0') || 0;
    });
...
export async function getErrorDistribution(
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to get error distribution: ${error.message}`);
    if (!data || data.length === 0) return [];
    ...
    (data as Partial<ModelRunLog>[]).forEach(log => {
      ...
      if (errorMsg.toLowerCase().includes('timeout')) category = 'Timeout';
      else if (errorMsg.toLowerCase().includes('rate limit')) category = 'Rate Limit';
      else if (errorMsg.toLowerCase().includes('auth')) category = 'Authentication';
      else if (errorMsg.toLowerCase().includes('network')) category = 'Network';
      else if (errorMsg.toLowerCase().includes('parse')) category = 'Parsing';
      else if (errorMsg.toLowerCase().includes('validation')) category = 'Validation';
