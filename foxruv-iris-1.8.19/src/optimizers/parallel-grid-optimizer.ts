...
export class ParallelGridOptimizer extends GridSearchOptimizer {
    ...
    private workersAvailable: boolean = true
    ...
    async optimize(
        ...
        const grid = (this as any).generateGrid(searchSpace, options?.maxTrials)
        ...
        try {
            ...
            const evaluationResults = await this.evaluateParallel(
                grid,
                evaluationFn,
                parallelism
            )
    ...
    private async evaluateParallel(
        ...
        const batches: ParameterConfiguration[][] = []
