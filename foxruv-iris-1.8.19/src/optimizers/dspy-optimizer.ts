...
export class DSPyOptimizer extends BaseOptimizer {
    ...
    async optimize(
        ...
        _evaluationFn: EvaluationFunction,
        ...
        const bestConfig: ParameterConfiguration = {
    ...
    private convertToDSPyRequest(
        ...
        const getFixedParam = (name: string): any => {
        ...
        const signature = getFixedParam('signature') as SignatureDefinition
        const training_data = getFixedParam('training_data') as TrainingExample[]

        if (!expert_role || !signature || !training_data) {
            throw new Error(
                'DSPyOptimizer requires "expert_role", "signature", and "training_data" as fixed parameters in SearchSpace'
            )
        }
