...
export class AxOptimizer extends BaseOptimizer {
    ...
    constructor(config: OptimizerConfig & { baseUrl?: string } = {}) {
        super(config)
    ...
    async optimize(
        ...
        if (!bestResult || bestResult.parameters === undefined || bestResult.score === undefined) {
            ...
            if (completedTrials.length === 0) {
                throw new Error(
                    `Optimization failed: All ${trials.length} trials failed and Ax service returned no best. ` +
                    `Last error: ${trials[trials.length - 1]?.error || 'Unknown'}`
                )
            }
            const localBest = completedTrials.reduce((best, trial) =>
    ...
    async resume(checkpointPath: string): Promise<OptimizationResult> {
        ...
        const { experiment_id } = await response.json() as { experiment_id: string }
    ...
    private async createExperiment(searchSpace: SearchSpace): Promise<string> {
    ...
    private async getNextTrial(experimentId: string): Promise<{
    ...
    private async completeTrial(
        ...
        const response = await fetch(
            `${this.baseUrl}/complete_trial/${experimentId}/${trialIndex}`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    private async getBest(experimentId: string): Promise<{
        parameters: ParameterConfiguration
        score: number
    }> {
    ...
    private async saveCheckpoint(experimentId: string): Promise<void> {
