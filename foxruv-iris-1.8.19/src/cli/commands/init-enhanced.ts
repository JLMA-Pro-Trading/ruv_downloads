...
export async function runEnhancedInit(
  projectRoot: string,
  options: EnhancedInitOptions = {}
): Promise<void> {
  const {
    createFoxruvFolder = true,
    createContexts = true,
    installDefaultMcps = false,
    enableAgentDB = true,
    enableSupabase = false,
    createGeminiMd = true, // Default to true
 ...
 * Display next steps
 */
function displayNextSteps(options: { showClaude?: boolean, showGemini?: boolean } = {}): void {
  console.log(chalk.blue(`
📚 Next Steps:
 ...
 */
async function createFoxruvFolderStructure(
  projectRoot: string,
  options: { enableAgentDB: boolean; enableSupabase: boolean }
): Promise<void> {
  ...
  if (!isTTY) console.log('- Creating .iris folder structure...');
  ...
  try {
    ...
    if (spinner) spinner.succeed('.iris folder created');
    else console.log('✔ .iris folder created');
  } catch (error) {
    if (spinner) spinner.fail('Failed to create .iris folder');
    else console.log('✗ Failed to create .iris folder');
    throw error;
  }
}
...
async function createSettingsJson(
  ...
  await fs.writeFile(
    path.join(foxruvRoot, 'config', 'settings.json'),
 ...
 */
async function createMcpServersJson(foxruvRoot: string): Promise<void> {
  const config = {
    servers: {},
  ...
  await fs.writeFile(
    path.join(foxruvRoot, 'config', 'mcp-servers.json'),
 ...
 */
async function createClaudeContextsJson(foxruvRoot: string): Promise<void> {
  const contexts = {
    active_contexts: [],
  ...
  await fs.writeFile(
    path.join(foxruvRoot, 'config', 'claude-contexts.json'),
 ...
 * Create MCP registry
 */
async function createMcpRegistryJson(foxruvRoot: string): Promise<void> {
  const registry = {
    version: '1.0.0',
    mcps: {
      // Development Tools
  ...
  await fs.writeFile(
    path.join(foxruvRoot, 'mcp', 'registry.json'),
 ...
 * Create .iris README
 */
async function createFoxruvReadme(foxruvRoot: string): Promise<void> {
  const readme = 
 ...
 * Create .gitignore for .iris
 */
async function createFoxruvGitignore(foxruvRoot: string): Promise<void> {
  const gitignore = 
 ...
 */
async function createContextStructure(projectRoot: string): Promise<void> {
  const isTTY = process.stdout.isTTY;
  const spinner = isTTY ? ora('Creating context-aware CLAUDE.md files...').start() : null;
  if (!isTTY) console.log('- Creating context-aware CLAUDE.md files...');
  ...
  try {
    ...
    for (const context of contexts) {
      ...
      if (spinner) spinner.text = `Created ${context.path}/CLAUDE.md`;
    }

    if (spinner) spinner.succeed('Context-aware CLAUDE.md files created');
    else console.log('✔ Context-aware CLAUDE.md files created');
  } catch (error) {
    if (spinner) spinner.fail('Failed to create context structure');
    else console.log('✗ Failed to create context structure');
    throw error;
  }
}
 ...
 */
async function detectProjectContexts(
  projectRoot: string
): Promise<Array<{ path: string; content: string; type: string }>> {
  const contexts: Array<{ path: string; content: string; type: string }> = [];
 ...
 */
function generateRootClaudeMd(): string {
  return `---
context_type: root
priority: 0
---
 ...
 */
function generateDatabaseClaudeMd(): string {
  return `---
 ...
 */
function generateApiClaudeMd(): string {
  return `---
context_type: api
extends: ../CLAUDE.md
priority: 10
 ...
 */
function generateMlClaudeMd(): string {
  return `---
context_type: ml
extends: ../CLAUDE.md
priority: 10
 ...
 */
async function installDefaultMCPs(): Promise<void> {
  const isTTY = process.stdout.isTTY;
  const spinner = isTTY ? ora('Installing recommended MCPs...').start() : null;
  if (!isTTY) console.log('- Installing recommended MCPs...');

  // This would call the MCP installation system
  // For now, just a placeholder

  if (spinner) spinner.info('MCP installation skipped (use npx iris mcp install)');
  else console.log('ℹ MCP installation skipped (use npx iris mcp install)');
}
 ...
 */
async function copyDocumentation(projectRoot: string): Promise<void> {
 ...
 */
async function writeOptimizationSkill(projectRoot: string): Promise<void> {
 ...
 */
async function writeGeminiOptimizationSkill(projectRoot: string): Promise<void> {
