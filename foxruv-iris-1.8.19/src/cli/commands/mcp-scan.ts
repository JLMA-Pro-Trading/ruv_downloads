...
async function scanMcpTools(
  ...
  return new Promise((resolve) => {
    const timeout = setTimeout(() => {
      console.log(chalk.yellow(`  ⏱️  ${mcpId}: Timeout (5s)`));
      resolve(null);
    }, 5000);
    ...
    try {
      ...
      child.on('error', (err) => {
        clearTimeout(timeout);
        if (!responded) {
          console.log(chalk.red(`  ❌ ${mcpId}: ${err.message}`));
          resolve(null);
        }
      });
      ...
      clearTimeout(timeout);
      console.log(chalk.red(`  ❌ ${mcpId}: Failed to spawn`));
      resolve(null);
    }
  });
}
 ...
 */
function generateSkillFromTools(mcpId: string, config: McpServerConfig, tools: McpTool[]): string {
...
export async function runMcpScan(options: {
  ...
  const results: { id: string; tools: number; status: string }[] = [];

  for (const mcpId of mcpIds) {
    const serverConfig = mcpServers[mcpId];
    const isDisabled = disabled.has(mcpId);
    
    console.log(`📡 ${chalk.cyan(mcpId)}${isDisabled ? chalk.gray(' (disabled)') : ''}`);
    ...
    if (tools && tools.length > 0) {
      ...
      console.log(chalk.green(`  ✅ ${tools.length} tool(s) → ${mcpId}.md`));
      results.push({ id: mcpId, tools: tools.length, status: 'success' });
    } else {
      console.log(chalk.yellow(`  ⚠️  Could not scan (MCP may need to be enabled first)`));
  ...
  console.log(chalk.green(`\n✅ Scanned ${successful.length}/${mcpIds.length} MCP(s)`));
  console.log(chalk.gray(`   Total tools documented: ${totalTools}`));
  console.log(chalk.gray(`   Skills saved to: ${outputDir}\n`));

  if (successful.length > 0) {
    console.log(chalk.blue('💡 Next steps:'));
    console.log(`   • Skills are now documented in ${chalk.cyan('.iris/mcp/skills/')}`);
    console.log(`   • Claude can read these even when MCPs are disabled`);
 ...
 */
export async function runMcpQuickScan(): Promise<void> {
  const config = await loadClaudeConfig();
  const projectPath = process.cwd();
  const projectConfig = config[projectPath];
  ...
  for (const [mcpId, serverConfig] of Object.entries(mcpServers)) {
    const isDisabled = disabled.has(mcpId);
    const status = isDisabled ? chalk.gray('○') : chalk.green('●');
