...
} from '../../migration/agentdb-to-supabase.js';
 ...
 */
export async function runTelemetryMigrate(options: TelemetryMigrateOptions = {}): Promise<MigrationResult> {
  ...
  const total =
    result.migratedRecords.signatures +
 ...
 */
export async function runTelemetrySync(options: TelemetrySyncOptions = {}): Promise<{
  success: boolean;
  synced: number;
  failed: number;
  pending: number;
  ...
  let beforeStats: TelemetrySummary = { queued: 0, sent: 0, failed: 0, lastFlush: null };
  ...
  let afterStats: TelemetrySummary = { queued: 0, sent: 0, failed: 0, lastFlush: null };
 ...
 */
export async function runTelemetryStatus(options: TelemetryStatusOptions = {}): Promise<void> {
  if (!options.json) {
    console.log(chalk.cyan.bold('\n--- Telemetry Status ---\n'));
  }
...
function determineHealth(
  ...
  if (!supabaseConfigured) return 'warning';
  if (!summary) return 'warning';
  if (summary.failed > summary.sent * 0.1) return 'error'; // >10% failure rate
  if (summary.queued > 100) return 'warning'; // Large queue
  return 'healthy';
}

function getHealthMessage(summary: TelemetrySummary | null, supabaseConfigured: boolean): string {
