...
export interface PatternDiscoveryConfig {
  ...
  useSupabase?: boolean // Use Supabase as primary backend (default: true)
  enableAgentDBCache?: boolean // Use AgentDB for local caching/acceleration (default: false)
}
...
export class PatternDiscovery {
  ...
  private config: Required<PatternDiscoveryConfig>
   ...
   */
  async learnPattern(pattern: Omit<LearnedPattern, 'patternId' | 'embedding'>): Promise<string> {
    ...
    const results = await Promise.allSettled([
      ...
      (async () => {
        ...
        stmt.run(
          ...
          pattern.performanceMetrics.successRate,
          pattern.performanceMetrics.avgConfidence,
          pattern.performanceMetrics.usageCount,
          JSON.stringify(pattern.performanceMetrics.domains),
          pattern.reusable ? 1 : 0,
  ...
  private async generatePatternEmbedding(
    pattern: Omit<LearnedPattern, 'patternId' | 'embedding'>
  ): Promise<number[]> {
  ...
  async recordUsage(
    ...
    usageStmt.run(
      ...
      Date.now(),
      success ? 1 : 0,
      adaptations ? JSON.stringify(adaptations) : null,
  ...
  private async updatePatternMetrics(patternId: string): Promise<void> {
    ...
    const stats = stmt.get(patternId) as { total: number; successful: number; domains: number }
    ...
    const domainRows = domainsStmt.all(patternId) as { domain: string }[]
  ...
  async findSimilarPatterns(
      ...
      this.useSupabase
        ? (async () => {
            ...
            for (const reflexion of similarReflexions) {
              const patternContext = reflexion.context as any
              if (!patternContext.patternId || !patternContext.reusable) continue

              const pattern: LearnedPattern = {
                patternId: patternContext.patternId,
                project: reflexion.project,
                domain: patternContext.domain || '',
                patternType: reflexion.reflexion_type.replace('pattern:', '') as any,
                ...
                performanceMetrics: {
                  successRate: (reflexion.outcome as any)?.successRate || reflexion.impact_score || 0,
                  avgConfidence: reflexion.confidence || 0,
                  usageCount: reflexion.reuse_count || 0,
                  domains: (reflexion.outcome as any)?.domains || [patternContext.domain]
                },
                reusable: true,
      ...
      this.agentDB
        ? (async () => {
            ...
            for (const expert of similarExperts) {
              ...
              if (!pattern || !pattern.reusable) continue
    ...
    const mergedMatches = [...cloudMatches, ...localMatches].filter(m => {
      if (seenIds.has(m.pattern.patternId)) return false
  ...
  private calculateContextOverlap(
    ...
    _targetContext: Record<string, any>
  ): number {
  ...
  private assessAdaptationNeeded(
    ...
    if (overlap > 0.9) return 'none'
    if (overlap > 0.7) return 'minor'
    if (overlap > 0.5) return 'moderate'
    return 'major'
  }
   ...
   */
  async generateTransferRecommendations(
    targetProject: string,
    ...
    for (const match of matches) {
      if (match.transferPotential < 0.6) continue

      const adaptationSteps = this.generateAdaptationSteps(
        match.pattern,
        match.adaptationRequired
      )
   ...
   */
  private extractRequiredContext(pattern: LearnedPattern): string[] {
   ...
   */
  async getPatternEvolution(patternId: string): Promise<PatternEvolution> {
    const pattern = await this.getPattern(patternId)
    ...
    const usageHistory = usageStmt.all(patternId) as any[]

    // Group by project/domain
    const crossDomainMap = new Map<string, any>()
    ...
    for (const usage of usageHistory) {
      ...
      if (usage.adaptations) {
        ...
        adapts.forEach((a: string) => stats.adaptations.add(a))
      }
    }
    ...
    const crossDomainUsage = Array.from(crossDomainMap.values()).map(stats => ({
      ...
      adaptations: Array.from(stats.adaptations) as string[]
    }))
   ...
   */
  async getProjectPatterns(project: string): Promise<LearnedPattern[]> {
      ...
      try {
        ...
        if (reflexions) {
          for (const reflexion of reflexions) {
            const patternContext = reflexion.context as any
            if (!patternContext.patternId) continue
 ...
 */
export function createPatternDiscovery(config?: PatternDiscoveryConfig): PatternDiscovery {
