...
export function getRequiredEnvVars(): RequiredEnvVars {
  ...
  if (!url) errors.push('FOXRUV_SUPABASE_URL is required');
  if (!serviceRoleKey) errors.push('FOXRUV_SUPABASE_SERVICE_ROLE_KEY is required');
  if (!projectId) errors.push('FOXRUV_PROJECT_ID is required');
 ...
 */
async function testDatabaseConnection(url: string, serviceRoleKey: string): Promise<void> {
 ...
 */
export async function validateConfigurationOrThrow(): Promise<void> {
