...
export class MCPWrapperGenerator {
  ...
  async generate(options: GeneratorOptions = {}): Promise<GeneratorResult> {
    ...
    try {
      ...
      if (servers.length === 0) {
        this.writer.getResult(dryRun).errors.push(
          'No MCP servers found. Please configure MCP servers in your project.'
        );
      ...
      if (serversToGenerate.length === 0) {
        this.writer.getResult(dryRun).errors.push(
          `No servers match filter: ${serverFilter.join(', ')}`
        );
        return this.writer.getResult(dryRun);
      }
   ...
   */
  private async generateServerWrapper(
    ...
    if (target === 'frontend' || target === 'both') {
      const frontendContext = { ...context, target: 'frontend' as const };
    ...
    if (target === 'backend' || target === 'both') {
      const backendContext = { ...context, target: 'backend' as const };
   ...
   */
  private generateServerIndex(serverName: string, target: string): string {
...
`;
   ...
   */
  private generateServerReadme(serverInfo: MCPServerInfo, target: string): string {
...
`;
   ...
   */
  private generateExampleUsage(serverInfo: MCPServerInfo): string {
...
console.log(result);`;
   ...
   */
  private generateExampleParams(schema: any): string {
    ...
    for (const [key, prop] of Object.entries(schema.properties as Record<string, any>)) {
      if (prop.enum) {
        ...
        switch (prop.type) {
          case 'string': params[key] = 'example'; break;
          case 'number': params[key] = 42; break;
          case 'boolean': params[key] = true; break;
          case 'array': params[key] = []; break;
   ...
   */
  private createContext(serverInfo: MCPServerInfo, target: 'frontend' | 'backend'): TemplateContext {
    return {
 ...
 */
export async function generateMCPWrappers(
