...
import { TemplateContext, MCPTool } from './types.js';
...
`;
   ...
   */
  private generateFrontendMethods(tools: MCPTool[], className: string): string {
   ...
   */
  private generateBackendMethods(tools: MCPTool[], className: string): string {
    return tools.map(tool => {
      const methodName = this.toMethodName(tool.name);
      const paramsType = this.generateParamsType(tool, className);
      const requiredParams = tool.inputSchema.required || [];
   ...
   */
  private generateParamsType(tool: MCPTool, className: string): string {
   ...
   * Generate tool types
   */
  private generateToolTypes(tools: MCPTool[]): string {
    return tools.map(tool => {
      if (!tool.inputSchema.properties) {
        return '';
      }
