  ...
  try {
    switch (name) {
      ...
      case 'iris_recommend_transfers': {
        const { sourceProjectId, targetProjectId } = args as {
        ...
        const recommendations = await irisPrime.findTransferablePatterns(
          sourceProjectId,
      ...
      case 'iris_get_expert_stats': {
        const { projectId, expertId, version } = args as {
          projectId?: string;
          expertId?: string;
          version?: string;
        };
        ...
        if (!projectId || !expertId || !version) {
          throw new McpError(
            ErrorCode.InvalidParams,
            'projectId, expertId, and version are required'
          );
        }
      ...
      case 'iris_consensus_lineage': {
        const { expertId, projectId } = args as {
          expertId: string;
          projectId?: string;
        };
      ...
      case 'iris_reflexion_search': {
        ...
        const reflexionMonitor = createReflexionMonitor({});
        const results = await reflexionMonitor.findSimilarReflexions(
          query,
          (limit || 10) / 100 // Convert limit to threshold
        );
      ...
      case 'iris_compare_reflexions': {
        const { reflexionId1, reflexionId2 } = args as {
          reflexionId1: string;
          reflexionId2: string;
        };
        ...
        if (!reflexionId1 || !reflexionId2) {
          throw new McpError(
            ErrorCode.InvalidParams,
            'reflexionId1 and reflexionId2 are required'
          );
        }
...
const PREBUILT_COMMANDS: Record<string, { description: string; command: string; args?: Record<string, string> }> = {
  'spawn-researchers': {
...
async function loadMcpSkills(): Promise<Record<string, string>> {
...
async function loadMcpConfigs(): Promise<Record<string, any>> {
