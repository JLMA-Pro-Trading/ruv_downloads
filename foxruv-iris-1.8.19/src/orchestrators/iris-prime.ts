...
import {
  ...
  type GlobalMetricsCollector,
  type PromptRegistry,
  type ReflexionMonitor,
  type ConsensusLineageTracker,
...
import type { IrisNotifier, IrisEvent } from '../notifications/types.js'
...
export interface ProjectConfig {
  ...
  autoRetrain: boolean
  autoPromote: boolean
  retrainingThreshold: number // Accuracy drop percentage to trigger retraining
  promotionThreshold: number // Accuracy improvement to promote new prompt
...
export class IrisPrime {
  ...
  private reflexionMonitor: ReflexionMonitor
  private consensusTracker: ConsensusLineageTracker
  private patternDiscovery: PatternDiscovery
  private config: Required<IrisPrimeConfig>
  private projectConfigs: Map<string, ProjectConfig> = new Map()
  private notifiers: IrisNotifier[]
   ...
   */
  private async emit(event: Omit<IrisEvent, 'runId' | 'createdAt'>): Promise<void> {
    const fullEvent: IrisEvent = { ...event, runId: this.currentRunId, createdAt: new Date() }
   ...
   */
  async evaluateProject(projectId: string): Promise<IrisReport> {
    ...
    const promptRecommendations: Array<{
      expertId: string
      currentVersion: string
      recommendedVersion: string
      expectedImprovement: number
    ...
    for (const metric of projectMetrics) {
      ...
      const evolution = await this.promptRegistry.getPromptEvolution(
        metric.expertId,
        projectId
      )
    ...
    for (const reflexion of reflexions) {
      const comparison = await this.reflexionMonitor.findSimilarReflexions(
        reflexion.id,
        0.7
      )
    ...
    const rotationRecommendations =
      await this.consensusTracker.generateRotationRecommendations(projectId)
   ...
   */
  async evaluateAllProjects(): Promise<CrossProjectReport> {
    ...
    for (const projectId of projects) {
      ...
      const criticalAlerts = report.driftAlerts.filter(
   ...
   */
  async autoPromotePrompts(projectId: string): Promise<Array<{
    expertId: string
    ...
    for (const metric of projectMetrics) {
      ...
      if (
        improvement >= projectConfig.promotionThreshold &&
        ...
        try {
          ...
          await storeExpertSignature(
            metric.expertId,
            best.version,
            'Promoted prompt from best-performing version', // Placeholder - actual prompt should be loaded
   ...
   */
  async autoRetrainExperts(projectId: string): Promise<Array<{
    expertId: string
    oldVersion: string
    newVersion: string
    improvement: number
    ...
    const retrainingNeeded = alerts.filter(
      a => a.triggerRetraining && a.severityLevel === 'critical'
    )
    ...
    for (const alert of retrainingNeeded) {
      ...
      if (results[0].success) {
        ...
        // Retire old version
        await this.consensusTracker.retireVersion(
          alert.expertId,
          alert.version,
          projectId,
          newVersion
        ...
        try {
          ...
          await storeExpertSignature(
            alert.expertId,
            newVersion,
            'Retrained prompt via IRIS auto-retraining',
            {}, // Placeholder signature - actual signature should be extracted from training results
            {
              performanceMetrics: {
          ...
          await recordSignatureUpgrade(
            ...
            alert.version,
            newVersion,
            `Auto-retrained due to ${alert.driftType} drift: ${(alert.percentageChange * 100).toFixed(1)}% change detected`,
            {
              improvement,
              previousAccuracy: alert.currentValue,
   ...
   */
  async findReusableReflexions(projectId: string): Promise<Array<{
    reflexionId: string
    sourceProject: string
   ...
   */
  async generateRotationReport(projectId: string): Promise<{
    ...
    const recommendations = await this.consensusTracker.generateRotationRecommendations(
      projectId
    )
  ...
  private generateRecommendedActions(params: {
    ...
    const highPriorityRotations = params.rotationRecommendations.filter(
      r => r.priority === 'high'
    )
 ...
 */
export function createIrisPrime(config?: IrisPrimeConfig): IrisPrime {
...
export const irisPrime = {
   ...
   */
  async autoPromotePrompts(projectId: string) {
    const iris = createIrisPrime()
    try {
   ...
   */
  async autoRetrainExperts(projectId: string) {
    const iris = createIrisPrime()
   ...
   */
  async findReusableReflexions(projectId: string) {
    const iris = createIrisPrime()
    try {
   ...
   */
  async findTransferablePatterns(projectId: string, context: Record<string, any>) {
    const iris = createIrisPrime()
    try {
   ...
   */
  async generateRotationReport(projectId: string) {
    const iris = createIrisPrime()
    try {
