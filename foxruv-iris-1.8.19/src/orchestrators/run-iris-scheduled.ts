...
import { createIrisPrime, type IrisPrime, type IrisReport, type CrossProjectReport } from './iris-prime.js'
...
interface ScheduleConfig {
  intervalMs: number // How often to run (default: 24 hours)
  projects: string[] // Projects to monitor
  logPath: string // Where to write reports
  autoRetrain: boolean // Enable auto-retraining
...
class ScheduledIrisRunner {
  ...
  async runOnce(): Promise<void> {
    ...
    const reportCache = new Map<string, IrisReport>()
   ...
   */
  private writeReport(name: string, report: IrisReport | CrossProjectReport): void {
   ...
   * Write summary log
   */
  private writeSummary(timestamp: Date, crossReport: CrossProjectReport): void {
   ...
   * Send webhook alert
   */
  private async sendWebhookAlert(projectId: string, report: IrisReport): Promise<void> {
   ...
   * Log to Supabase
   */
  private async logToSupabase(report: IrisReport): Promise<void> {
