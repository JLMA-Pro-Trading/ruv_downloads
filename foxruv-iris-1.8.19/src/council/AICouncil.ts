...
export class AICouncil {
  ...
  private promptScientist: PromptScientist
  ...
  // Configuration
  private config: Required<AICouncilConfig>
   ...
   */
  async holdMeeting(telemetry: CouncilTelemetryInput): Promise<CouncilMeetingResult> {
   ...
   */
  private async runPatternMasterAnalysis(
   ...
   */
  private async runPromptScientistAnalysis(
    telemetry: CouncilTelemetryInput
  ): Promise<AgentAnalysis> {
   ...
   */
  private async runPerformanceJudgeAnalysis(
    telemetry: CouncilTelemetryInput
  ): Promise<AgentAnalysis> {
   ...
   */
  private async runTransferTesterAnalysis(
    telemetry: CouncilTelemetryInput
  ): Promise<AgentAnalysis> {
   ...
   */
  private async runSafetyValidatorAnalysis(
    telemetry: CouncilTelemetryInput
  ): Promise<AgentAnalysis> {
  ...
  private generateDecisions(
    ...
    if (patternAnalysis && 'transferCandidates' in patternAnalysis) {
      for (const candidate of patternAnalysis.transferCandidates.slice(0, 1)) {
        // Top candidate only
        const decision: PatternTransferDecision = {
    ...
    if (promptAnalysis && 'deploymentCandidates' in promptAnalysis) {
      for (const candidate of promptAnalysis.deploymentCandidates.slice(0, 1)) {
        ...
        const decision: PromptUpgradeDecision = {
          type: 'prompt_upgrade',
    ...
    if (performanceAnalysis && 'rotationRecommendations' in performanceAnalysis) {
      for (const rec of performanceAnalysis.rotationRecommendations.slice(0, 1)) {
        const decision: ExpertRotationDecision = {
      ...
      switch (decision.type) {
        case 'pattern_transfer':
          topRecommendations.push(
            `Transfer "${decision.pattern.name}" from ${decision.pattern.sourceProject} to ${decision.targetProjects.join(', ')}`
          )
          break
        case 'prompt_upgrade':
          topRecommendations.push(
            `Upgrade ${decision.prompt.expertType} prompts to v${decision.prompt.version}`
          )
          break
        case 'expert_rotation':
          topRecommendations.push(
            `${decision.action} ${decision.sourceExpert.expertId} in ${decision.sourceExpert.project}`
          )
          break
      }
    }
   ...
   */
  async executeDecisions(result: CouncilMeetingResult): Promise<void> {
 ...
 * Create AI Council
 */
export function createAICouncil(config?: AICouncilConfig): AICouncil {
