...
export interface PromptScientistConfig {
  minImprovementThreshold?: number // Minimum improvement to recommend
  minTestProjects?: number // Minimum projects to test on
  voteWeight?: number
}
...
export class PromptScientist {
  private config: Required<PromptScientistConfig>
   ...
   */
  async analyze(telemetry: CouncilTelemetryInput): Promise<PromptAnalysis> {
   ...
   */
  private identifyTopExperts(telemetry: CouncilTelemetryInput): Array<{
    project: string
    expertId: string
    expertType: string
    version: string
  ...
  private async evolvePrompts(
    ...
    const expertsByType = new Map<string, typeof topExperts>()
    ...
    for (const [expertType, experts] of expertsByType) {
      if (experts.length < 2) continue // Need at least 2 for crossover
   ...
   */
  private generateMutations(_expert: any): Array<{
 ...
 */
export function createPromptScientist(config?: PromptScientistConfig): PromptScientist {
