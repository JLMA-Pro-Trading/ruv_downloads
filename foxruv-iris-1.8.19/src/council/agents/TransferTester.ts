...
export interface TransferTesterConfig {
  testPercentage?: number // Traffic percentage for A/B test
  testDuration?: string // Test duration
  minImprovement?: number // Minimum improvement to pass
  voteWeight?: number
}
...
export class TransferTester {
  private config: Required<TransferTesterConfig>
   ...
   */
  async analyze(telemetry: CouncilTelemetryInput): Promise<TransferTestAnalysis> {
   ...
   */
  private async runTransferTests(
  ...
  private findCompatibleProjects(
    ...
    for (const projectData of telemetry.projects) {
      ...
      const hasOverlap = pattern.domains.some(domain =>
        projectData.experts.some(e => e.expertType.includes(domain) || domain.includes(e.expertType))
      )
  ...
  private simulateABTest(
    ...
    const compatibleExpert = projectData.experts.find(e =>
      pattern.domains.some(d => e.expertType.includes(d))
    )
 ...
 */
export function createTransferTester(config?: TransferTesterConfig): TransferTester {
