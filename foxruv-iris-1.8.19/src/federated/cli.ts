...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const results = await controlPlane.testPattern(
        options.pattern,
        [options.target]
      );
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      metricsTable.push(
        ['Total Telemetry Events', chalk.cyan(metrics.totalTelemetryEvents)],
        ['Patterns Detected', chalk.cyan(metrics.patternsDetected)],
        ['Decisions Proposed', chalk.cyan(metrics.decisionsProposed)],
        ['Decisions Approved', chalk.green(metrics.decisionsApproved)],
        ['Decisions Executed', chalk.green(metrics.decisionsExecuted)],
        ['Patterns Transferred', chalk.green(metrics.patternsTransferred)],
