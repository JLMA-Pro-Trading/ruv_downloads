...
interface TriggerConfig {
  ...
  // Time window for event counting (ms)
  timeWindow: number
  // Cooldown period between invocations (ms)
  cooldownPeriod: number
  // Critical events that always trigger
 ...
 */
function parseArgs(): AutoInvokeOptions | null {
  ...
  const options: Partial<AutoInvokeOptions> = {}
  ...
  return options as AutoInvokeOptions
}
 ...
 */
function getEventHistoryPath(dbBasePath: string): string {
 ...
 */
function getLastInvocationPath(dbBasePath: string): string {
 ...
 */
function recordEvent(dbBasePath: string, event: EventRecord): void {
 ...
 * Read event history
 */
function readEventHistory(dbBasePath: string, timeWindow: number): EventRecord[] {
  const historyPath = getEventHistoryPath(dbBasePath)
 ...
 */
function getLastInvocation(dbBasePath: string, project: string): number | null {
 ...
 */
function recordInvocation(dbBasePath: string, project: string, timestamp: number): void {
  ensureDir(dbBasePath)
  const lastInvocationPath = getLastInvocationPath(dbBasePath)
...
function checkTriggers(
  ...
  const projectEvents = eventHistory.filter(
    e => e.project === project && e.event === event
  )

  const threshold = (config.eventThresholds as any)[event] || 5
