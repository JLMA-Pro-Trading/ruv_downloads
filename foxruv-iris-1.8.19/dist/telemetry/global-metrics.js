 ...
 * @version 2.0.0
 */
import { logTelemetry, getExpertStats, detectDrift } from '../supabase/telemetry.js';
...
export class GlobalMetricsCollector {
    ...
    async logEvent(event) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
            stmt.run(event.project, event.expertId, event.version, event.timestamp.getTime(), event.confidence, event.outcome, event.durationMs, event.reflexionUsed ? 1 : 0, event.consensusParticipation ? 1 : 0, event.metadata ? JSON.stringify(event.metadata) : null);
        }
     ...
     */
    async updateExpertMetrics(project, expertId, version) {
        const db = await this.getDb();
        if (!db)
            return; // No local cache to update
      ...
      LIMIT 1000
    `);
        const stats = stmt.get(safeBindValue(project), safeBindValue(expertId), safeBindValue(version));
    ...
    `);
        upsertStmt.run(project, expertId, version, stats.total, stats.correct, accuracy, stats.avg_confidence || 0, stats.avg_duration || 0, trend, Date.now());
    }
    /**
     * Calculate performance trend
     */
    async calculateTrend(db, project, expertId, version) {
        if (!db)
            return 'stable';
    ...
    `);
        const recent = recentStmt.get(project, expertId, version).accuracy || 0;
        const older = olderStmt.get(project, expertId, version).accuracy || 0;
     ...
     */
    async checkForDrift(project, expertId, version) {
        ...
        if (this.useSupabase) {
            try {
                ...
                if (driftResult.driftDetected) {
                    ...
                    await this.createDriftAlert(project, expertId, version, 'confidence', severityLevel, message, driftResult.recentConfidence, driftResult.baselineConfidence, driftResult.confidenceDrop / 100, driftResult.recommendation ? [driftResult.recommendation] : []);
                }
    ...
    `);
        ...
        const baseline = baselineStmt.get(safeBindValue(project), safeBindValue(expertId), safeBindValue(version), cutoff, cutoff + halfWindow);
        const recent = recentStmt.get(safeBindValue(project), safeBindValue(expertId), safeBindValue(version), cutoff + halfWindow);
     ...
     */
    async checkMetricDrift(project, expertId, version, metricType, baselineValue, currentValue) {
        if (!baselineValue || !currentValue)
            return;
        const percentageChange = Math.abs((currentValue - baselineValue) / baselineValue);
        if (percentageChange < this.config.driftThreshold)
            return;
        // Drift detected!
        let severityLevel;
        if (percentageChange < 0.2)
            severityLevel = 'info';
        else if (percentageChange < 0.3)
        ...
        await this.createDriftAlert(project, expertId, version, metricType, severityLevel, message, currentValue, baselineValue, percentageChange, recommendations);
    }
    /**
     * Create drift alert (stores in AgentDB only - Supabase doesn't have drift_alerts table)
     */
    async createDriftAlert(project, expertId, version, driftType, severityLevel, message, currentValue, baselineValue, percentageChange, recommendations) {
        const db = await this.getDb();
        if (!db)
            return;
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(alertId, project, expertId, version, severityLevel, driftType, message, currentValue, baselineValue, percentageChange, Date.now(), JSON.stringify(recommendations), triggerRetraining ? 1 : 0);
    }
    /**
     * Generate recommendations based on drift
     */
    generateDriftRecommendations(metricType, currentValue, baselineValue) {
     ...
     */
    async getExpertMetrics(project, expertId, version) {
            ...
            try {
                ...
                if (stats.totalRuns > 0) {
                    return {
                        ...
                        recentTrend: 'stable', // TODO: Calculate from Supabase
      ...
      WHERE project = ? AND expert_id = ? AND version = ?
    `);
        const row = stmt.get(safeBindValue(project), safeBindValue(expertId), safeBindValue(version));
        if (!row)
            return null;
        return {
                ...
                if (experts && experts.length > 0) {
                    ...
                    const metricsPromises = experts.map(async (expert) => {
                        ...
                        return {
                            ...
                            avgConfidence: stats.avgConfidence,
                            avgDuration: stats.avgLatencyMs,
                            recentTrend: 'stable', // TODO: Calculate trend from Supabase
                            lastUpdated: new Date()
                        };
                    });
    ...
    `);
        const rows = stmt.all(safeBindValue(project));
        return rows.map(row => ({
            expertId: row.expert_id,
            project: row.project,
            version: row.version,
     ...
     */
    async getCrossProjectMetrics(expertType) {
        const db = await this.getDb();
        if (!db) {
            return {
                expertType,
    ...
    `);
        const stats = stmt.get(safeBindValue(`%${expertType}%`));
      ...
      LIMIT 1
    `);
        const best = bestStmt.get(safeBindValue(`%${expertType}%`));
        // Get recent alerts
    ...
    `);
        const windowMs = this.config.driftWindow * 24 * 60 * 60 * 1000;
        const alertCount = alertStmt.get(Date.now() - windowMs).count;
        return {
            expertType,
     ...
     */
    async getUnacknowledgedAlerts(project) {
        const db = await this.getDb();
        if (!db)
            return [];
        const query = project
