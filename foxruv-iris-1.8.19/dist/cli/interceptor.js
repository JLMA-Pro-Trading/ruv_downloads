 ...
 */
export async function interceptCommand(commandName, commandFn, ...args) {
 ...
 */
export function withSmartDefaults(commandFn) {
    return async (...args) => {
 ...
 */
export async function isSmartExecutionEnabled() {
    const engine = getExecutionEngine();
    const config = await engine.loadConfig();
 ...
 */
export async function showSmartExecutionBanner() {
    const engine = getExecutionEngine();
    const config = await engine.loadConfig();
    if (config.use_agentic_flow || config.use_agentdb) {
        const features = [];
