...
export async function runMcpList(options) {
    ...
    if (options.search) {
        ...
        mcps = mcps.filter(([, mcp]) => mcp.name.toLowerCase().includes(search) ||
 ...
 * Install MCP server
 */
export async function runMcpInstall(mcpId, options = {}) {
    console.log(chalk.blue(`\n🔧 Installing ${mcpId}...\n`));
 ...
 * Validate security
 */
async function validateSecurity(mcp) {
 ...
 * Install npm package
 */
async function installNpmPackage(mcp) {
 ...
 */
async function configureEnvironment(mcp) {
 ...
 */
async function generateSkillDocs(mcpId, mcp) {
 ...
 */
async function generateWrappers(mcpId, mcp) {
 ...
 */
async function updateMcpConfig(mcpId, mcp) {
    const spinner = ora('Updating configuration...').start();
    try {
        const configPath = path.join(process.cwd(), '.iris', 'config', 'mcp-servers.json');
        let config = { servers: {}, global_mcps_disabled: false };
        try {
 ...
 */
async function trackInstallation(mcpId, mcp) {
    try {
 ...
 * Display usage instructions
 */
function displayUsageInstructions(mcpId, mcp) {
