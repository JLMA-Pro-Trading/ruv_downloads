...
export async function runPromptBoostStatus(projectPath) {
...
export async function runPromptBoostOn(projectPath) {
    const config = loadConfig(projectPath);
    config.enabled = true;
...
export async function runPromptBoostOff(projectPath) {
    const config = loadConfig(projectPath);
...
export async function runPromptBoostShadow(projectPath, enable) {
    const config = loadConfig(projectPath);
...
export async function runPromptBoostSetModel(projectPath, model, options) {
    const config = loadConfig(projectPath);
    ...
    if (options.type) {
        config.experimentalImprove.localModel = options.type;
    }
    saveConfig(projectPath, config);
...
export async function runPromptBoostReview(projectPath) {
    const experimentsFile = path.join(projectPath, EXPERIMENTS_PATH);
    if (!fs.existsSync(experimentsFile)) {
 ...
 */
export function logExperiment(projectPath, original, improved, context) {
    const experimentsFile = path.join(projectPath, EXPERIMENTS_PATH);
 ...
 */
export function getInjectionText(config) {
 ...
 */
export async function improvePromptWithLocalModel(prompt, config) {
