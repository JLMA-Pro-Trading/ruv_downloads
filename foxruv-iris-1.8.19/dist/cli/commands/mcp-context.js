 ...
 */
async function loadProjectMcpJson(projectPath) {
    try {
 ...
 */
async function getAllProjectMcps(projectPath) {
 ...
 */
export async function runMcpContextList() {
 ...
 */
export async function runMcpContextDisable(mcpIds) {
 ...
 */
export async function runMcpContextEnable(mcpIds) {
    const { raw, projects } = await loadClaudeConfig();
    const projectPath = getCurrentProjectPath();
    if (!projects[projectPath]) {
        console.log(chalk.yellow('No project config found. Run this in a Claude Code project.\n'));
...
export async function runMcpContextOptimize(options = {}) {
    ...
    if (options.interactive !== false) {
        ...
        selected.forEach((id) => console.log(chalk.green(`   • ${id}`)));
    }
    else {
 ...
 */
export async function runMcpContextSummary() {
    const { projects } = await loadClaudeConfig();
