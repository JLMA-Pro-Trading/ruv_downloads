...
export class SupabaseMcpTracker {
    ...
    async getSkillStats(skillId, projectName) {
        try {
            if (!isSupabaseInitialized())
                return null;
            const client = getSupabase();
            ...
            const stats = {
                ...
                tools: data.reduce((acc, d) => {
                    ...
                    acc[tool].count++;
                    if (d.success)
                        acc[tool].successCount++;
                    if (d.latency_ms)
                        acc[tool].latencies.push(d.latency_ms);
                    return acc;
                }, {}),
                recentErrors: data
                    .filter((d) => !d.success)
    ...
    async getDriftDetection(skillId, windowDays = 7) {
        try {
            if (!isSupabaseInitialized())
                return null;
            const client = getSupabase();
...
let supabaseTracker = null;
export function getSupabaseMcpTracker() {
    if (!supabaseTracker) {
