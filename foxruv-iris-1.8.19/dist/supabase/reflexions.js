 ...
 */
export async function saveReflexion(reflexionType, context, outcome, success, options) {
    return await withRetry(async () => {
        ...
        const { data, error } = await supabase
            ...
            .single();
        if (error)
            throw new Error(`Failed to save reflexion: ${error.message}`);
        return data;
 ...
 */
export async function findSimilarReflexions(_embedding, options) {
    return await withRetry(async () => {
        const supabase = getSupabase();
        const project = getProjectId();
        ...
        if (error)
            throw new Error(`Failed to find similar reflexions: ${error.message}`);
        return data || [];
    }, { maxRetries: 3, timeoutMs: 30000 });
}
/**
 ...
 */
export async function getSuccessfulReflexions(reflexionType, options) {
    return await withRetry(async () => {
        const supabase = getSupabase();
        const project = getProjectId();
        let query = supabase
        ...
        if (error)
            throw new Error(`Failed to get successful reflexions: ${error.message}`);
        return data || [];
    }, { maxRetries: 3, timeoutMs: 30000 });
}
/**
 ...
 */
export async function markReflexionReused(reflexionId) {
    return await withRetry(async () => {
        const supabase = getSupabase();
 ...
 */
export async function getReflexionStats(reflexionType) {
    return await withRetry(async () => {
        const supabase = getSupabase();
        const project = getProjectId();
        let query = supabase
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to get reflexion stats: ${error.message}`);
        ...
        const avgImpactScore = data.reduce((sum, r) => sum + (r.impact_score || 0), 0) / total;
