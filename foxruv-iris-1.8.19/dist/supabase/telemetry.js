 ...
 * ```
 */
export async function getExpertStats(expertId, options) {
    ...
    const avgConfidence = data.reduce((sum, r) => sum + (r.confidence || 0), 0) / totalRuns;
    const avgLatencyMs = data.reduce((sum, r) => sum + (r.latency_ms || 0), 0) / totalRuns;
    ...
    const successfulRuns = data.filter((r) => r.outcome === 'success' && !r.error_message).length;
    const successRate = successfulRuns / totalRuns;
    const reflexionUsageRate = data.filter((r) => r.reflexion_used).length / totalRuns;
    return {
        totalRuns,
        avgConfidence,
        avgLatencyMs,
 ...
 * ```
 */
export async function getRecentLogs(expertId, options) {
 ...
 * ```
 */
export async function detectDrift(expertId, version, options) {
    ...
    const baselineConfidence = baselineData.reduce((sum, r) => sum + (r.confidence ?? 0), 0) / baselineData.length;
    const recentConfidence = recentData.reduce((sum, r) => sum + (r.confidence ?? 0), 0) / recentData.length;
 ...
 */
export async function getProjectExpertStats(projectId, options) {
    // Return empty array if Supabase not initialized
    if (!isSupabaseInitialized()) {
        return [];
    }
    ...
    const { data, error } = await query;
    if (error)
        throw new Error(`Failed to get project expert stats: ${error.message}`);
    if (!data || data.length === 0)
        return [];
    // Group by expert
    ...
    const expertStats = Array.from(expertMap.entries()).map(([expertId, logs]) => {
        ...
        const avgConfidence = logs.reduce((sum, l) => sum + (l.confidence || 0), 0) / totalCalls;
        const avgLatency = logs.reduce((sum, l) => sum + (l.latency_ms || 0), 0) / totalCalls;
 ...
 */
export async function getExpertPerformanceTrends(projectId, expertId, hours = 24) {
    // Return empty array if Supabase not initialized
    if (!isSupabaseInitialized()) {
        return [];
    }
    ...
    if (error)
        throw new Error(`Failed to get expert performance trends: ${error.message}`);
    if (!data || data.length === 0)
        return [];
    // Group by hour
    ...
    const trends = Array.from(hourlyMap.entries()).map(([time, logs]) => {
        const avgConfidence = logs.reduce((sum, l) => sum + (l.confidence || 0), 0) / logs.length;
        const avgLatency = logs.reduce((sum, l) => sum + (l.latency_ms || 0), 0) / logs.length;
        return {
            time,
            accuracy: avgConfidence,
            latency: avgLatency,
