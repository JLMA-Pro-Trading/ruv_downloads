 ...
 */
export async function storeIrisReport(reportType, healthScore, overallHealth, reportData, options) {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error)
        throw new Error(`Failed to store IRIS report: ${error.message}`);
    return data;
}
/**
 * Get the most recent IRIS report for a project
 */
export async function getLatestIrisReport(projectId, reportType) {
    const supabase = getSupabase();
    ...
    if (error)
        throw new Error(`Failed to get latest IRIS report: ${error.message}`);
    return data && data.length > 0 ? data[0] : null;
}
/**
 * Get IRIS report history for a project
 */
export async function getIrisReportHistory(options) {
    const supabase = getSupabase();
    const project = options?.projectId || getProjectId();
    let query = supabase
        .from('iris_reports')
    ...
    const { data, error } = await query;
    if (error)
        throw new Error(`Failed to get IRIS report history: ${error.message}`);
    return data || [];
}
/**
 * Get IRIS report summary and trends
 */
export async function getIrisReportSummary(projectId, lookbackDays = 30) {
    const supabase = getSupabase();
    const project = projectId || getProjectId();
    ...
    const { data, error } = await supabase
        ...
        .order('created_at', { ascending: false });
    if (error)
        throw new Error(`Failed to get IRIS report summary: ${error.message}`);
 ...
 */
export async function getCriticalReports(options) {
    const supabase = getSupabase();
    let query = supabase
        .from('iris_reports')
        .select('*')
    ...
    const { data, error } = await query;
    if (error)
        throw new Error(`Failed to get critical reports: ${error.message}`);
    return data || [];
}
/**
 * Compare health across projects
 */
export async function compareProjectHealth(projectIds) {
    const supabase = getSupabase();
    let query = supabase
        .from('iris_reports')
        .select('*');
    ...
    if (error)
        throw new Error(`Failed to compare project health: ${error.message}`);
    if (!data || data.length === 0)
        return [];
    // Group by project
 ...
 */
export async function deleteOldIrisReports(daysToKeep = 90, projectId) {
    const supabase = getSupabase();
    ...
    if (error)
        throw new Error(`Failed to delete old IRIS reports: ${error.message}`);
    return count || 0;
}
/**
...
export async function getAllProjectsSummary() {
    ...
    const { data: reports, error } = await supabase
        ...
        .order('created_at', { ascending: false });
    if (error)
        throw new Error(`Failed to get projects summary: ${error.message}`);
    ...
    const projectSummaries = await Promise.all(Array.from(projectMap.entries()).map(async ([project, projectReports]) => {
 ...
 */
export function transformReportToProject(report) {
    return {
        id: report.id,
        name: report.project,
