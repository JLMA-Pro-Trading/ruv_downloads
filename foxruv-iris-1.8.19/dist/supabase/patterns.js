 ...
 */
export async function storePattern(patternName, patternType, patternData, options) {
...
export async function findPatterns(options) {
    ...
    if (error)
        throw new Error(`Failed to find patterns: ${error.message}`);
    return data || [];
}
/**
 ...
 */
export async function findSimilarPatterns(_queryEmbedding, options) {
    const supabase = getSupabase();
    const project = getProjectId();
    ...
    const matches = (data || []).map(pattern => ({
        pattern,
 ...
 */
export async function markPatternUsed(patternId) {
    const supabase = getSupabase();
 ...
 */
export async function updatePatternSuccessRate(patternId, newSuccessRate) {
    const supabase = getSupabase();
    const { error } = await supabase
        ...
        .eq('id', patternId);
    if (error)
        throw new Error(`Failed to update pattern success rate: ${error.message}`);
}
/**
...
export async function getCrossProjectPatterns(options) {
    ...
    const { data, error } = await query;
    if (error)
        throw new Error(`Failed to get cross-project patterns: ${error.message}`);
    return data || [];
}
