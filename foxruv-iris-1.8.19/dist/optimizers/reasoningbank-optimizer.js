...
import { ReasoningBankManager } from '../storage/reasoning-bank.js';
/**
...
export class ReasoningBankOptimizer extends BaseOptimizer {
    ...
    async optimize(searchSpace, evaluationFn, options) {
        ...
        const explorationConfigs = this.generateExplorationConfigs(searchSpace, maxTrials - warmStartConfigs.length);
        // Step 3: Combine warm-start and exploration
        const allConfigs = [...warmStartConfigs, ...explorationConfigs];
        if (this.config.verbose) {
            console.log(`   💡 Warm-start configs: ${warmStartConfigs.length}, ` +
        ...
        for (let i = 0; i < allConfigs.length; i++) {
            ...
            try {
                ...
                if (score.primary > this.currentBestScore) {
                    ...
                    if (this.config.verbose) {
                        console.log(`   ✨ New best (${isWarmStart ? 'warm-start' : 'exploration'}): ` +
                ...
                if (options?.earlyStoppingPatience) {
                    ...
                    const improvements = recentTrials.filter(t => t.score.primary > (trials[trials.length - patience - 1]?.score.primary ?? -Infinity));
     ...
     */
    async generateWarmStartConfigs(searchSpace, maxTrials) {
        ...
        try {
            ...
            const highConfidenceTrajectories = await this.reasoningBank.getTrajectories(this.expertRole, { verdict: 'success' });
            const warmStartConfigs = [];
            ...
            if (this.config.verbose && warmStartConfigs.length > 0) {
                console.log(`   🎯 Found ${warmStartConfigs.length} warm-start configs from ` +
     ...
     */
    extractConfigFromTrajectory(trajectory, searchSpace) {
        try {
     ...
     */
    createVariation(baseConfig, searchSpace) {
     ...
     */
    generateExplorationConfigs(searchSpace, count) {
        if (count <= 0) {
            return [];
        }
        const configs = [];
     ...
     */
    async storeSuccessfulTrial(config, score, isWarmStart) {
        try {
            const trajectory = {
     ...
     */
    async storeFailedTrial(config, error) {
        try {
            const trajectory = {
