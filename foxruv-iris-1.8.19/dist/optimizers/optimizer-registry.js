...
export class OptimizerRegistry {
     ...
     */
    static register(name, OptimizerClass) {
    ...
    static async get(name, config) {
        ...
        if (!healthy) {
            ...
            console.warn(`⚠️  Optimizer '${name}' failed health check (missing dependencies: ${metadata.dependencies.join(', ')})`);
            return null;
        }
        // Cache
     ...
     */
    static async getBestAvailable(preferences, config) {
