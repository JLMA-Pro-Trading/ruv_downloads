 ...
 */
export async function migrateSignatures(options) {
    ...
    try {
        ...
        // Get total count
        const totalResult = db.prepare('SELECT COUNT(*) as count FROM expert_signatures').get();
 ...
 */
export async function migrateReflexions(options) {
    const { agentDbPath, dryRun = false, batchSize = 100, onProgress } = options;
    let db = null;
    let migratedCount = 0;
    const errors = [];
    try {
        ...
        // Get total count
        const totalResult = db.prepare('SELECT COUNT(*) as count FROM reflexions').get();
        const total = totalResult.count;
        if (total === 0) {
 ...
 */
export async function migrateTelemetry(options) {
    const { agentDbPath, dryRun = false, batchSize = 100, onProgress } = options;
    let db = null;
    let migratedCount = 0;
    const errors = [];
    try {
        ...
        // Get total count
        const totalResult = db.prepare('SELECT COUNT(*) as count FROM model_telemetry').get();
        const total = totalResult.count;
        if (total === 0) {
 ...
 */
export async function migrateConsensus(options) {
    const { agentDbPath, dryRun = false, batchSize = 100, onProgress } = options;
    let db = null;
    let migratedCount = 0;
    const errors = [];
    try {
        ...
        // Get total count
        const totalResult = db.prepare('SELECT COUNT(*) as count FROM consensus_decisions').get();
        const total = totalResult.count;
        if (total === 0) {
        ...
        for (let i = 0; i < consensusRecords.length; i += batchSize) {
            ...
            for (const cons of batch) {
                try {
                    ...
                    if (!dryRun) {
                        await recordConsensusLineage(cons.decision_id || 'migrated', Array.isArray(votes) ? votes : [], { winning_option: cons.winning_option }, cons.confidence, {
                            metadata: {
                                ...metadata,
                                migrated_from_agentdb: true,
                                original_id: cons.id,
