...
const DB_PATH = path.join(os.homedir(), '.nvm/versions/node/v22.16.0/lib/node_modules/@foxruv/iris/data/iris/global-metrics.db');
/**
 * Log telemetry directly to AgentDB (bypasses all imports)
 */
export async function logTelemetryLocal(event) {
    ...
    `);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run('nfl-predictor', event.expertId, event.version || '1.0.0', Math.floor(Date.now() / 1000), event.confidence || 0.0, event.outcome || 'success', event.latencyMs || 0, 0, // reflexion_used
 ...
 */
export function withTelemetry(expertId, version = '1.0.0') {
