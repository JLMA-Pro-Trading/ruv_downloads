...
import { getSupabase, isSupabaseInitialized, getTenantId } from '../supabase/client.js';
// ============================================================================
// Main Class
// ============================================================================
/**
...
export class IrisContextCollector {
    ...
    async recordEvent(event) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
            stmt.run(eventId, event.eventType, event.project, event.expertId || null, event.timestamp.getTime(), event.severity, JSON.stringify(event.eventData), event.contextType, event.tags ? JSON.stringify(event.tags) : null, aggKey, event.windowStart?.getTime() || null, event.windowEnd?.getTime() || null);
        }
    ...
    async recordPerformanceMetrics(metrics) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(metrics.project, metrics.expertId, metrics.version, metrics.timestamp.getTime(), metrics.windowSize, metrics.successRate, metrics.avgConfidence, metrics.avgLatencyMs, metrics.totalPredictions, metrics.driftScore, metrics.qualityScore, metrics.baselineSuccessRate || null, metrics.baselineConfidence || null, metrics.percentageChange || null, metrics.totalTokensUsed, metrics.avgTokensPerPrediction, metrics.metadata ? JSON.stringify(metrics.metadata) : null);
        // Update cache
    ...
    async registerTrigger(trigger) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(trigger.triggerId, trigger.name, trigger.description || null, trigger.enabled ? 1 : 0, trigger.priority, trigger.conditionType, JSON.stringify(trigger.conditionSpec), trigger.projectFilter || null, trigger.expertFilter || null, trigger.actionType, trigger.actionConfig ? JSON.stringify(trigger.actionConfig) : null, trigger.cooldownSeconds);
     ...
     */
    async getActiveTriggers(project) {
        const db = await this.getDb();
        if (!db)
            return [];
        const query = project
     ...
     */
    async getRecentEvents(eventType, project, expertId, limit = 100) {
        const db = await this.getDb();
        if (!db)
            return [];
     ...
     */
    async getEventsInWindow(project, windowStart, windowEnd, filters) {
        const db = await this.getDb();
        if (!db)
            return [];
        let query = `SELECT * FROM iris_context_events WHERE project = ? AND timestamp >= ? AND timestamp <= ?`;
        const params = [project, windowStart.getTime(), windowEnd.getTime()];
     ...
     */
    async getLatestMetrics(project, expertId, version) {
        // Check cache first
      ...
      LIMIT 1
    `);
        const row = stmt.get(project, expertId, version);
        if (!row)
            return null;
 ...
 */
export function createIrisContextCollector(config) {
