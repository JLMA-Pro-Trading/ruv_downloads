...
export class PerformanceJudge {
    ...
    async analyze(telemetry) {
        ...
        const rotationRecommendations = this.generateRotationRecommendations(driftingExperts, topPerformers);
        // Generate vote
        const { recommendation, confidence } = this.generateRecommendation(driftingExperts, rotationRecommendations);
        return {
     ...
     * Identify drifting experts
     */
    identifyDrift(leagues, telemetry) {
        const drifting = [];
        for (const entries of leagues.values()) {
            for (const entry of entries) {
                // Check if expert has drift alert
                const hasDriftAlert = telemetry.alerts.some(alert => alert.expertId === entry.expertId &&
     ...
     * Generate rotation recommendations
     */
    generateRotationRecommendations(driftingExperts, topPerformers) {
     ...
     */
    generateRecommendation(driftingExperts, _rotationRecommendations) {
 ...
 */
export function createPerformanceJudge(config) {
