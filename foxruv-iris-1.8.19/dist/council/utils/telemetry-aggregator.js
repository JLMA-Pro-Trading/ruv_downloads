 ...
 */
export async function aggregateTelemetryForCouncil(projects, timeWindow, metricsCollector, patternDiscovery) {
    const projectData = [];
    ...
    for (const alert of alerts) {
        for (const project of projectData) {
            const expert = project.experts.find((e) => e.expertId === alert.expertId);
            if (expert) {
                expert.drift = {
                    detected: true,
                    severity: alert.severityLevel,
 ...
 */
function inferExpertType(expertId) {
    const lowerId = expertId.toLowerCase();
    if (lowerId.includes('analyst'))
        return 'analyst';
    if (lowerId.includes('predictor'))
        return 'predictor';
    if (lowerId.includes('optimizer'))
        return 'optimizer';
    if (lowerId.includes('researcher'))
        return 'researcher';
    if (lowerId.includes('reviewer'))
        return 'reviewer';
    return 'general';
}
/**
 ...
 */
async function discoverPatterns(projectData, _patternDiscovery) {
    const patterns = [];
