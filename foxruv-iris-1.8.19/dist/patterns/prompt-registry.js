...
import { storeExpertSignature, loadActiveExpertSignature, getSignatureHistory, recordSignatureUpgrade, getSupabase, getProjectId, isSupabaseInitialized, } from '../supabase/index.js';
/**
 * Prompt Registry
 */
export class PromptRegistry {
     ...
     */
    async registerSignature(signature, options) {
        ...
        if (this.useSupabase) {
            try {
                ...
                await storeExpertSignature(signature.expertId, signature.version, signature.prompt, supabaseSignature, {
                    metadata: {
                        ...signature.metadata,
                ...
                if (previousVersion && previousVersion !== signature.version) {
                    await recordSignatureUpgrade(signature.expertId, previousVersion, signature.version, options?.changelog || `Upgraded from ${previousVersion} to ${signature.version}`, options?.improvementMetrics);
                }
            }
            catch (error) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
            stmt.run(signatureId, signature.expertId, signature.expertRole, signature.project, signature.version, signature.prompt, JSON.stringify(signature.inputFields), JSON.stringify(signature.outputFields), signature.deployedAt.getTime(), Date.now(), signature.metadata ? JSON.stringify(signature.metadata) : null);
        }
        return signatureId;
    }
    /**
     ...
     */
    async recordEvaluation(signatureId, accuracy, confidence, latencyMs, context) {
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `);
            evalStmt.run(signatureId, Date.now(), accuracy, confidence, latencyMs, context ? JSON.stringify(context) : null);
     ...
     */
    async updateSignatureMetrics(signatureId) {
        if (!this.db)
            return;
      ...
      WHERE signature_id = ?
    `);
        const metrics = stmt.get(signatureId);
      ...
      WHERE signature_id = ?
    `);
        updateStmt.run(metrics.avg_accuracy || 0, metrics.avg_confidence || 0, metrics.avg_latency || 0, metrics.total || 0, Date.now(), signatureId);
    }
    /**
     * Deprecate a signature
     */
    async deprecateSignature(signatureId) {
     ...
     */
    async getLatestSignature(expertId, project) {
     ...
     */
    async getBestAcrossProjects(expertRole) {
        ...
        LIMIT 1
      `);
            const row = stmt.get(expertRole, this.config.minEvaluationsForValidity);
            if (!row)
                return null;
            return {
     ...
     */
    async findSimilarSignatures(signatureId, threshold = 0.7) {
        if (!this.db) {
    ...
    `);
        const candidates = stmt.all(source.expertRole, signatureId);
     ...
     */
    async getPromptEvolution(expertId, project) {
        let signatures = [];
        // Primary: Get from Supabase
        if (this.useSupabase) {
            try {
        ...
        if (signatures.length === 0 && this.db) {
           ...
           ORDER BY deployed_at ASC`;
            ...
            signatures = rows.map(row => this.mapRowToSignature(row));
        }
        ...
        // Find best version
        const bestVersion = versions.reduce((best, v) => v.accuracy > best.accuracy ? v : best).version;
        return {
            expertId,
    ...
    async getSignature(signatureId) {
      ...
      `);
            const row = stmt.get(signatureId);
            return row ? this.mapRowToSignature(row) : null;
        }
        return null;
    }
     ...
     */
    async getProjectSignatures(project, includeDeprecated = false) {
        // Primary: Get from Supabase
        if (this.useSupabase) {
            try {
                const supabase = getSupabase();
 ...
 */
export function createPromptRegistry(config) {
