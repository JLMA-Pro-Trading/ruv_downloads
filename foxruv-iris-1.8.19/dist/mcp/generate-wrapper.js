 ...
 */
function generateWrapper(serverName, server) {
    const safeName = serverName.replace(/[^a-zA-Z0-9_]/g, '_');
    const className = safeName.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
    return `
/**
 ...
 */
function generateIndex(serverNames) {
 ...
 */
function generateWrappers(outputDir = './src/mcp/wrappers') {
