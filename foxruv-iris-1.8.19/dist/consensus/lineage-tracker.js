...
import { recordConsensusLineage, getConsensusHistory, getExpertParticipationStats, isSupabaseInitialized, } from '../supabase/index.js';
/**
 * Consensus Lineage Tracker
 */
export class ConsensusLineageTracker {
     ...
     */
    async recordConsensus(decision) {
        const contributingExperts = decision.participants.map(p => ({
            expertId: p.expertId,
            version: p.version,
            vote: p.prediction,
        ...
        const winningParticipant = decision.participants.reduce((prev, current) => current.weight > prev.weight ? current : prev);
        ...
        const results = await Promise.allSettled([
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
                decisionStmt.run(decision.decisionId, decision.project, decision.timestamp.getTime(), JSON.stringify(decision.finalDecision), decision.consensusScore, decision.outcome || 'pending', decision.metadata ? JSON.stringify(decision.metadata) : null);
        ...
        `);
                for (const participant of decision.participants) {
                    participationStmt.run(decision.decisionId, participant.expertId, participant.version, participant.project, participant.confidence, JSON.stringify(participant.prediction), JSON.stringify(participant.reasoning), participant.weight);
                    await this.updateVersionLineage(participant.expertId, participant.version, participant.project, decision.outcome === 'correct');
                }
                await this.updateConsensusPattern(decision);
            })(),
            // Write to Supabase (cloud, cross-project)
            recordConsensusLineage(decision.decisionId, contributingExperts, decision.finalDecision, decision.consensusScore, {
    ...
    `);
        ...
        for (const participant of participants) {
            await this.updateVersionLineage(participant.expertId, participant.version, participant.project, outcome === 'correct');
        }
     ...
     */
    async updateVersionLineage(expertId, version, project, wasSuccessful) {
        const db = await this.getDb();
        if (!db)
            return;
    ...
    `);
      ...
      `);
        ...
        VALUES (?, ?, ?, ?, 1, ?, ?)
      `);
            insertStmt.run(expertId, version, project, Date.now(), wasSuccessful ? 1 : 0, wasSuccessful ? 1.0 : 0.0);
        }
    }
    /**
     ...
     */
    async getVersionLineage(expertId, project) {
        ...
        const [supabaseResults, localResults] = await Promise.allSettled([
            ...
            (async () => {
                const consensusHistory = await getConsensusHistory(project || 'default', 1000);
                const versionMap = new Map();
                for (const consensus of consensusHistory) {
                    const expertContribution = consensus.contributing_experts.find((e) => (e.expertId || e.expert_id) === expertId);
                    ...
                    const wasSuccessful = JSON.stringify(expertContribution.vote) === JSON.stringify(consensus.final_decision) ||
        ...
        return mergedLineage.sort((a, b) => b.deployedAt.getTime() - a.deployedAt.getTime());
    }
    /**
     * Mark version as retired
     */
    async retireVersion(expertId, version, project, replacedBy) {
        const db = await this.getDb();
        if (!db)
            return;
        const stmt = db.prepare(`
     ...
     */
    async updateConsensusPattern(decision) {
    ...
    `);
      ...
      `);
        ...
        VALUES (?, ?, ?, ?, ?)
      `);
            insertStmt.run(patternId, JSON.stringify(expertIds), JSON.stringify(versions), JSON.stringify([decision.project]), Date.now());
        }
        // Recalculate metrics
     ...
     */
    async recalculatePatternMetrics(decisionId) {
        // Get decision
        ...
        const avgConsensusScore = patternDecisions.reduce((sum, d) => sum + d.consensusScore, 0) / totalDecisions;
        const db = await this.getDb();
        if (!db)
            return;
        // Update pattern
     ...
     */
    async getConsensusPatterns(minOccurrences) {
        const db = await this.getDb();
        if (!db)
            return [];
     ...
     */
    async analyzeVersionImpact(expertId, oldVersion, newVersion, project) {
        ...
        let significance;
        if (Math.abs(accuracyChange) > 0.1)
            significance = 'major';
        else if (Math.abs(accuracyChange) > 0.05)
     ...
     * Get version metrics
     */
    async getVersionMetrics(expertId, version, project, windowMs) {
        const db = await this.getDb();
        if (!db) {
            return {
                accuracy: 0,
      ...
      WHERE p.expert_id = ? AND p.version = ? AND p.project = ?
        AND d.timestamp >= ?
    `);
        const rows = stmt.all(expertId, version, project, cutoff);
        if (rows.length === 0) {
     ...
     */
    async generateRotationRecommendations(project) {
        ...
        try {
            ...
            for (const consensus of consensusHistory) {
                for (const expert of consensus.contributing_experts) {
                    activeExperts.add(expert.expertId || expert.expert_id);
                }
            }
    ...
    `);
        const activeVersions = stmt.all(project);
     ...
     */
    async getDecisionParticipants(decisionId) {
        const db = await this.getDb();
        if (!db)
            return [];
        const stmt = db.prepare(`
    ...
    `);
        const rows = stmt.all(decisionId);
        return rows.map(row => ({
            expertId: row.expert_id,
            version: row.version,
            project: row.project,
     ...
     */
    async getPatternDecisions(patternId) {
      ...
      LIMIT 1000
    `);
        const decisionIds = stmt.all().map(r => r.decision_id);
 ...
 */
export function createConsensusLineageTracker(config) {
