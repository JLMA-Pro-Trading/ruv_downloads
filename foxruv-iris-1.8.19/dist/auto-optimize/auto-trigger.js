...
export function ensureTelemetryDir(projectPath = process.cwd()) {
 ...
 */
export function recordTelemetry(projectPath, record) {
 ...
 */
export function getTelemetryForTarget(projectPath, target) {
    const telemetryFile = path.join(projectPath, TELEMETRY_DIR, 'calls.json');
    if (!fs.existsSync(telemetryFile)) {
        return [];
    }
    try {
        const records = JSON.parse(fs.readFileSync(telemetryFile, 'utf-8'));
 ...
 */
export function getAllTargets(projectPath) {
    const telemetryFile = path.join(projectPath, TELEMETRY_DIR, 'calls.json');
    if (!fs.existsSync(telemetryFile)) {
        return [];
    }
 ...
 */
export function shouldTriggerOptimization(projectPath, target, config = DEFAULT_CONFIG) {
    if (!config.enabled) {
        return null;
    }
    ...
    if (fs.existsSync(triggersFile)) {
        try {
            ...
            const lastTrigger = triggers.find((t) => t.target === target);
            if (lastTrigger) {
 ...
 */
export function logOptimizationTrigger(projectPath, trigger) {
    const triggersFile = path.join(projectPath, TRIGGERS_LOG);
    let triggers = [];
    try {
        if (fs.existsSync(triggersFile)) {
 ...
 */
export function checkAllTargets(projectPath, config = DEFAULT_CONFIG) {
 ...
 */
export async function runAutoTriggerCheck(projectPath = process.cwd(), options = {}) {
 ...
 */
export async function recordAndCheck(projectPath, target, success, latencyMs, metadata) {
