 ...
 */
export async function buildDailyDigest(projects, iris, crossReport, reportCache) {
 ...
 */
export function formatDailyDigest(digest) {
    const lines = [];
    // Header
    ...
    if (!digest.promotions.length) {
        ...
        for (const p of digest.promotions.slice(0, 3)) {
            lines.push(`✨ ${p.expertId} (${p.project}): ${p.oldVersion} → ${p.newVersion}`);
        }
    ...
    if (!digest.patterns.length) {
        ...
        for (const pat of digest.patterns.slice(0, 3)) {
            lines.push(`🔁 ${pat.fromProject} → ${pat.toProject}: "${pat.name}" ` +
 ...
 */
export function formatProjectStatus(projectName, report) {
    const lines = [];
 ...
 */
export function formatDriftSummary(reports) {
    const lines = [];
 ...
 */
export function formatPatternsSummary(patterns) {
    const lines = [];
 ...
 */
export function formatMultiProjectStatus(reports) {
    const lines = [];
    lines.push('🧠 *IRIS Multi-Project Status*');
    lines.push('');
    for (const report of reports) {
        const icon = report.overallHealth === 'excellent'
            ? '🟢'
