...
class TelemetryAggregator {
    config;
    metricsCollector;
    constructor(config, metricsCollector = new GlobalMetricsCollector()) {
        this.config = config;
...
export class FederatedControlPlane extends EventEmitter {
    ...
    async executeDecision(decision) {
        ...
        try {
            ...
            const successfulTargets = testResults.filter((r) => r.success);
     ...
     */
    async pushToProject(project, payload) {
     ...
     */
    getEligibleTargets(sourceProject) {
     ...
     */
    async markPatternAsUniversal(patternId) {
        await this.supabase
            .from('patterns')
            .update({
            status: 'universal',
     ...
     */
    async analyzePattern(patternId, targetProjects) {
    ...
    buildAdvisorDecisions(advice) {
        if (!advice?.recommendations || advice.recommendations.length === 0)
            return [];
        return advice.recommendations.map((rec, idx) => ({
     ...
     */
    async testPattern(patternId, targetProjects) {
