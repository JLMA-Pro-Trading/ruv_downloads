...
class CodeScanner {
    ...
    findSourceFiles(projectPath) {
        ...
        if (this.languages.includes('typescript'))
            extensions.add('.ts');
        if (this.languages.includes('javascript'))
            extensions.add('.js');
        if (this.languages.includes('python'))
            extensions.add('.py');
        const walk = (dir) => {
     ...
     */
    scanTypeScript(filePath, projectPath, content, _lines) {
        const experts = [];
     ...
     * Scan Python file
     */
    scanPython(filePath, projectPath, content, _lines) {
        const experts = [];
        const relativePath = path.relative(projectPath, filePath);
     ...
     */
    findPythonClassEnd(content, startPos) {
     ...
     */
    findPythonFunctionEnd(content, startPos) {
...
class DiscoveryStorage {
    ...
    async storeDiscoveredExpert(expert) {
        ...
        if (this.useSupabase) {
            try {
                await saveReflexion(`discovery:${expert.expertType}`, {
                    name: expert.name,
                    ...
                    signature: expert.signature,
                    hasTelemetry: expert.hasTelemetry
                }, { confidence: expert.confidence }, true, {
                    expertId: expert.id,
                    embedding,
     ...
     */
    async markInstrumented(expertId, changes) {
     ...
     */
    async generateEmbedding(expert) {
...
class InteractiveCLI {
    async presentDiscoveries(experts) {
        console.log('\n' + '='.repeat(80));
    ...
    async askInstrumentationApproval(missing) {
        if (missing.length === 0) {
