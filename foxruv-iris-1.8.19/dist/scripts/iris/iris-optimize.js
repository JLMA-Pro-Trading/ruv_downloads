...
async function importUserScript(scriptPath) {
    const absolutePath = path.resolve(process.cwd(), scriptPath);
    return await import(absolutePath);
}
export default async function optimize(cmdOptions) {
    ...
    try {
        ...
        const optimizer = await OptimizerRegistry.getBestAvailable(preferences, optimizerConfig);
        ...
        const evaluationFn = async (config) => {
