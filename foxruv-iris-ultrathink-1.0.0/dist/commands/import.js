...
var importCommand = new Command("import").description("Import MCP servers from Claude Desktop settings as skills").option("--backup", "Create backup of existing skills directory").option("--disable-global", "Remove from global Claude settings after import").option("--dry-run", "Show what would be imported without making changes").option("-s, --source <path>", "Path to Claude settings.json").option("-o, --output <path>", "Output skills directory", "./skills").action(async (options) => {
...
${skills.map((skill) => `- **${skill.name}** (${skill.type}) - ${skill.enabled ? "\u2713 Enabled" : "\u2717 Disabled"}`).join("\n")}
