...
export declare abstract class Optimizer<TInput = any, TOutput = any> {
     ...
     */
    abstract compile<T1 = TInput, T2 = TOutput>(program: Module<T1, T2>, trainset: TrainingExample<T1, T2>[] | Array<T1 & Partial<T2>>, valset?: TrainingExample<T1, T2>[] | Array<T1 & Partial<T2>>): Promise<Module<T1, T2> | OptimizationResult<T1, T2>>;
    /**
