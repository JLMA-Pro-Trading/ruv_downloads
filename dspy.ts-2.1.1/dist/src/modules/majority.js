...
exports.weightedMajority = weightedMajority;
exports.consensusMajority = consensusMajority;
/**
 ...
 * ```
 */
async function majority(module, input, options = {}) {
    const n = options.n || 5;
 ...
 * Group predictions by similarity
 */
function groupPredictions(predictions, equalityFn, hashFn) {
 ...
 */
async function weightedMajority(module, input, options = {}) {
    ...
    const weighted = result.predictions.map((pred) => {
        const avgConfidence = pred.output.confidence !== undefined
            ? pred.output.confidence
            : 1.0;
        return Object.assign(Object.assign({}, pred), { weightedCount: pred.count * avgConfidence });
    }).sort((a, b) => b.weightedCount - a.weightedCount);
    const totalWeight = weighted.reduce((sum, p) => sum + p.weightedCount, 0);
    const winner = weighted[0];
    return Object.assign(Object.assign({}, result), { output: winner.output, confidence: winner.weightedCount / totalWeight });
}
/**
 * Consensus threshold voting (require minimum agreement)
 */
async function consensusMajority(module, input, threshold = 0.6, options = {}) {
    const result = await majority(module, input, options);
...
//# sourceMappingURL=majority.js.map