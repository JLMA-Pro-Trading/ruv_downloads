...
const safla_1 = require("./safla");
class ReasoningBank {
    constructor(agentDB, saflaConfig) {
        this.units = new Map();
        ...
        this.safla = new safla_1.SAFLA(saflaConfig);
    }
    /**
    ...
    async retrieve(query) {
        this.ensureInitialized();
        try {
            const { context, minConfidence = 0.5, successfulOnly = false, transferableOnly = false, limit = 10, } = query;
    ...
    async learnFromExperience(experience) {
        ...
        try {
            ...
            if (similar) {
                ...
                unit = this.safla.updateFromExperience(similar, experience.success, experience.feedback);
    ...
    getStats() {
        ...
        const avgConfidence = units.reduce((sum, u) => sum + u.confidence, 0) / units.length || 0;
        const avgSuccessRate = units.reduce((sum, u) => sum + u.successRate, 0) / units.length || 0;
        return {
     ...
     */
    createKnowledgeUnit(experience) {
        var _a, _b;
        const now = new Date();
        return {
            id: this.generateId(),
            ...
            confidence: ((_a = experience.feedback) === null || _a === void 0 ? void 0 : _a.score) || (experience.success ? 0.7 : 0.3),
            usageCount: 1,
            successRate: experience.success ? 1.0 : 0.0,
            lessons: (_b = experience.feedback) === null || _b === void 0 ? void 0 : _b.comments,
            relatedUnits: [],
            createdAt: now,
            updatedAt: now,
            metadata: {},
     ...
     */
    assessTransferability(experience) {
     ...
     */
    calculateSimilarity(unit, context) {
        let score = 0;
        let factors = 0;
        // Domain match
        ...
        const intersection = unitFeatures.filter((f) => contextFeatures.includes(f));
     ...
     */
    generateExplanation(unit, context) {
        const parts = [];
        if (unit.context.domain === context.domain) {
