...
exports.questionAnsweringExample = questionAnsweringExample;
exports.problemSolvingExample = problemSolvingExample;
const react_1 = require("../../src/modules/react");
const openai_1 = require("../../src/lm/providers/openai");
...
const wikipediaTool = {
    ...
    execute: async (query) => {
        ...
        const mockData = {
            ...
            'albert einstein': 'Albert Einstein (1879-1955) was a German-born theoretical physicist who developed the theory of relativity.',
...
async function questionAnsweringExample() {
    ...
    const lm = new openai_1.OpenAILM({
        apiKey,
        model: 'gpt-3.5-turbo',
        defaultOptions: {
            temperature: 0.7,
    ...
    await lm.init();
    (0, core_1.configureLM)(lm);
    // Create ReAct agent with tools
    const agent = new react_1.ReAct({
        name: 'QAAgent',
        signature: {
            inputs: [
                {
...
async function problemSolvingExample() {
    ...
    const lm = new openai_1.OpenAILM({ apiKey });
    await lm.init();
    (0, core_1.configureLM)(lm);
    const agent = new react_1.ReAct({
        name: 'ProblemSolver',
    ...
    const problem = 'Einstein developed the theory of relativity. If he published his special relativity paper in 1905 and lived for 76 years, what year did he die? Also calculate his age at death.';
    ...
    try {
        ...
        console.log(`ReActTool Uses: ${result.steps.filter((s) => s.type === 'action').length}`);
    }
    catch (error) {
        console.error('Error:', error);
    }
