...
export class GeneticOptimizer {
     ...
     */
    async evolve(initialPopulation, fitnessFunction) {
        // Initialize population with efficient structure
        let population = this.initializePopulation(initialPopulation);
        ...
        for (let gen = 0; gen < this.config.generations; gen++) {
            ...
            if (this.config.maxFitnessEvaluations &&
                this.fitnessCache.getStats().size >= this.config.maxFitnessEvaluations) {
    ...
    initializePopulation(seeds) {
        return seeds.map((content, _index) => ({
            id: this.generateId(),
            content,
            generation: 0,
            fitness: 0,
    ...
    async crossover(parent1, parent2) {
        const strategy = this.config.crossoverOperations[Math.floor(Math.random() * this.config.crossoverOperations.length)];
        let content;
        switch (strategy) {
            case 'uniform':
    ...
    async mutate(individual) {
        const strategy = this.config.mutationStrategies[Math.floor(Math.random() * this.config.mutationStrategies.length)];
        let content;
    ...
    hasConverged() {
        if (!this.config.convergenceThreshold)
            return false;
        ...
        return improvementRate < this.config.convergenceThreshold;
    }
    // Utility functions
    generateId() {
